...
その有用性は素晴らしいものですが、このダッシュボードにはまだいくつかの機能が不足しています。
...
スクリーンショットのように7, 14, 28 daysの3択しかなく、「導入から4か月でどれだけ浸透したか」あるいは特定の週の利用状況といった分析はできません。
...
定点観測を続けることは新たな気づきを生みます。月次でダッシュボードを確認し、Active User数の推移をウォッチします。当社のGitHub Copilotライセンスは希望者のみに付与しています。しかしながら導入したものの使われていない、そんな状況を早期に検知できるのが嬉しいポイントです。
...
## GUIでは取得できない詳細はCopilot Metrics APIを利用する

ダッシュボードで提供される情報だけでは物足りない場合、[Copilot Metrics API](https://docs.github.com/en/enterprise-cloud@latest/rest/copilot/copilot-metrics?apiVersion=2022-11-28)を活用することで、より詳細なデータを取得できます。

### APIで取得できる情報

Copilot Metrics APIを使うと、以下のような詳細なメトリクスを取得できます。以下はドキュメントから引用し日本語に意訳したものです。

- 過去100日分のデータ: GUIの最大28日間に対し、APIでは100日分のデータにアクセス可能
- Active UsersとEngaged Usersの内訳: より詳細なユーザー分類
- 言語別・IDE別の内訳: 各プログラミング言語やエディタごとの利用状況
- Enterprise/Organization/Team単位での取得: 階層ごとの柔軟なメトリクス取得

### GUIとAPIの比較

GUIとAPIの違いを表にまとめたものが以下です。APIの柔軟性が明確になります。

| 項目 | GUI（ダッシュボード） | API |
|------|----------------------|-----|
| データ期間 | 最大28日間 | 最大100日間 |
| 時系列分析 | 7/14/28日の3択のみ | 日単位で柔軟に取得可能 |
| データ加工 | そのまま表示 | 自由に加工・分析可能 |
| 外部連携 | 不可 | BIツールやスプレッドシートへ連携可能 |

### APIの活用例

より柔軟性の高いAPIを活用することで、GUIでは実現できない以下のような分析が可能になります。

- 複数月にまたがった月次レポートの自動生成: 100日分のデータを使った長期トレンド分析
- カスタムダッシュボードの構築: New RelicやBIツールなどへのデータ連携
- アラート設定: 利用率が一定以下に落ちた場合の通知

このような違いからGUIのダッシュボードはクイックな状況確認に、APIはより深い分析や他システムとの連携に、それぞれ使い分けることができます。

ユーザの希望としては手軽なGUIのダッシュボードがより柔軟に機能拡張されていくことですが、あらかじめ柔軟性のあるAPIが用意されている点はさすがはGitHubでしょう。利用するこちらの腕が試されますね。

## おわりに

以上が「GitHub Copilot usage metrics dashboardに感じるAIガバナンス」でした。

日進月歩で進化するAIツールの導入は「入れて終わり」ではなく、継続的に利用状況を把握し改善していくことが大切です。今回紹介した「GitHub Copilot usage metrics dashboard」は、まさにそのための土台となる機能だと感じています。
